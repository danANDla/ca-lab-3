...
public class RightRect extends RiemannMethod {

    public RightRect(EquationManager equationManager, boolean debug) {
    ...
    private double getSquare(Equation eq, double[] borders) {
        if (eq.getImage(borders[0]) * eq.getImage(borders[1]) >= 0) {
            return Math.abs(borders[1] - borders[0]) * eq.getImage(borders[1]);
        }
        double t = borders[1];
